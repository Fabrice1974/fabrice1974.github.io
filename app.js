/* ============================================================
   RED Monitor — app.js — v4.0.8 (restore stable)
   ============================================================ */
...
function diffCount(cur, prev){ var s=new Set(prev||[]), c=0; cur.forEach(id=>{if(!s.has(id)) c++;}); return c; }
...
function computeNewlyDetectedCountAndPersist(items){
  ...
  newlyDetectedCount=diffCount(cur, seen);
  saveSeenIds(cur);
}
...
function renderCard(reg){
  ...
  return '<div class="card-reg card-reg-'+reg.cat+'">'
    ...
    + '<div id="summary-'+reg.id+'" class="summary-box'+(isOpen?'':' hidden')+'"><p style="font-size:12px;color:#c0c4d8;line-height:1.7;margin-bottom:8px">'+esc(reg.summary||'')+'</p>'+linkBtn+'</div>'
    + '</div>';
}
...
function renderAccueil(){
  var agenda = AGENDA.map(function(e){
    return '<a href="'+e.link+'" target="_blank" rel="noopener" style="text-decoration:none">'
      + '<div class="agenda-row"><div class="agenda-date"><p style="font-size:12px;font-weight:800;color:#a78bfa;margin:0">'+esc(e.date.slice(0,5))+'</p><p style="font-size:10px;color:#a78bfa;margin:0">'+esc(e.date.slice(6))+'</p></div>'
      + '<div style="flex:1"><p style="font-size:12px;font-weight:600;color:#e8eaf0;margin:0">'+esc(e.flags)+' '+esc(e.label)+'</p><p style="font-size:10px;color:#4a7dff;margin-top:2px">Voir le texte</p></div></div></a>';
  }).join('');

  document.getElementById('tab-accueil').innerHTML =
    '<div style="padding:14px 16px 90px">'
    + '<div class="card card-green"><p class="fw7 fs12 t-green">'+DATA_FILTRE.length+' textes surveillés — échéances après 01/06/2026</p></div>'
    ...
    + '<p class="section-label t-muted">CALENDRIER DES ÉCHÉANCES</p>' + agenda
    + '</div>';
}
...
function renderVeille(){
  ...
  document.getElementById('tab-veille').innerHTML =
    '<div style="padding:14px 16px 90px"><div style="display:flex;gap:6px;flex-wrap:wrap;margin-bottom:16px">'+filterBtns+'</div>'+html+'</div>';
}

function renderAlertes(){
  document.getElementById('tab-alertes').innerHTML =
    '<div style="padding:14px 16px 90px">'
    + '<div class="card-plain"><p class="fw7 fs12 t-text mb8">Statut scraping</p><p class="fs11 t-muted">Dernier scan : <span class="t-green">'+lastScan+'</span></p><p class="fs11 t-muted">Prochain scan : <span class="t-warn">'+nextScan+'</span></p></div>'
    + '</div>';
}
...
function rebuildFromDynamic(dynamicItems){
  ...
  var newOnly=(dynamicItems||[]).map(normalizeDynamicItem).filter(d=>!ids.includes(d.id));
...
document.addEventListener('DOMContentLoaded', function(){
  ...
  if(bell) bell.addEventListener('click', ()=>setTab('alertes'));
  if(navA) navA.addEventListener('click', ()=>setTab('accueil'));
  if(navV) navV.addEventListener('click', ()=>setTab('veille'));
  if(navAl) navAl.addEventListener('click', ()=>setTab('alertes'));
